...
if(isset($_GET['action']))
{
    switch($_GET['action'])
    {
		...
		case 'media_preview':
			...
			$var_one = $_SERVER['SERVER_NAME'];
			$var_two = explode("/",$_SERVER['REQUEST_URI']);

			if( $var_two[1] == 'app' )
			{
				$urll = $var_one.'/'.$var_two[1].'/'.$var_two[2];
			}
			else
			{
				$urll = $var_one.'/'.$var_two[1];
			}

			$dir_fix = str_replace('../', NULL, $preview_file);

			$full_path = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! 
			...
			$ext_file = pathinfo($_GET['file'], PATHINFO_EXTENSION);
			ob_start();
			?>

			<?php if($ext_file == 'mp4'): ?>

			<b>Judul : <?php echo $_GET['title']; ?></b>
			<br>
			<?php header('Content-type', 'audio/mpeg'); ?>
			<video controls autoplay style="width:100%; height:95%;">
				<source src="<?php echo $preview_file; ?>" type="audio/<?php echo $ext_file; ?>">
				Browser anda tidak support untuk memutar ini.
			</video> 
			...
			<b>Judul : <?php echo $_GET['title']; ?></b>
			<br>
			<?php header('Content-type', 'video/mp4'); ?>

			<audio controls autoplay style="width:100%; height:50%;">
				<source src="<?php echo $preview_file; ?>" type="audio/<?php echo $ext_file; ?>">
				Browser anda tidak support untuk memutar ini.
			</audio> 

			<?php endif; ?>
			<?php //$ext = strtolower(pathinfo($preview_file, PATHINFO_EXTENSION)); ?>
			<?php //if($ext != 'mp4'): ?>
				<!--<h5>Video Support : <b>Mp4 , Ogg, Webm</b></h5>-->
				<?php //echo $ext; ?>
			<?php //else: ?>
				<!--<video width="100%" height="500" autoplay>
					<source src="<?php echo $preview_file; ?>" type="<?php echo 'video/mp4' ?>">
					Your browser does not support the video tag.
				</video>--> 
			<?php //endif; ?>
			<?php //echo $dir_fix; ?>
			<!--<div id="jp_container_1" class="jp-video " style="margin:0 auto;overflow:auto;">
